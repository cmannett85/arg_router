...
struct ar::parser<fs::path> {
    [[nodiscard]] static inline fs::path parse(std::string_view arg)
    {
        return arg;
    }
};
...
int main(int argc, char* argv[])
{
    const auto common_args = ar::list{
        ar::flag(arp::long_name<S_("force")>,
                 arp::short_name<'f'>,
                 arp::description<S_("Force overwrite existing files")>),
        ar::positional_arg<fs::path>(
            arp::required,
    ...
    ar::root(
        ...
        ar::mode(arp::none_name<S_("copy")>,
                 ...
                 ar::positional_arg<std::vector<fs::path>>(
                     ...
                     arp::min_count<1>),
                 arp::router{
                     [](bool force, fs::path dest, std::vector<fs::path> srcs) {
