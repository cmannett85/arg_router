...
public:
    ...
    std::optional<ValueType> parse_phase(std::string_view token, const Parents&... parents) const
    {
        ...
        utility::tuple_type_iterator<typename stub_node::policies_type>(  //
            [&](auto i) {
                ...
                if constexpr (policy::has_parse_phase_method_v<this_policy, ValueType> &&
                              traits::is_specialisation_of_v<this_policy, policy::custom_parser>) {
                    BOOST_CHECK(!result);
                    result = this->this_policy::template parse_phase<ValueType>(token, parents...);
                }
            });
...
{
    static_assert(policy::is_policy_v<policy::custom_parser<int>>, "Policy test has failed");
}

BOOST_AUTO_TEST_CASE(parse_phase_test)
{
    const auto root = stub_node{
        stub_node{policy::custom_parser<int>{[](auto str) { return parser<int>::parse(str); }}},
    ...
    auto f = [&](auto token, const auto& owner, auto expected_value) {
        ...
        const auto result = owner.template parse_phase<value_type>(token, owner, root);
    ...
    test::data_set(
        f,
        std::tuple{
            std::tuple{"42", std::get<0>(root.children()), std::optional<int>{42}},
            std::tuple{"42", std::get<1>(root.children()), std::optional<std::string_view>{"42"}},
            std::tuple{"42", std::get<2>(root.children()), std::optional<std::string_view>{}}});
}
