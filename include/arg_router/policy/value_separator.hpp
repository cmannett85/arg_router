 ...
 * token(s).
 *
 * Your terminal will separate tokens using whitespace by default, but often a
 * different character is used e.g. <TT>--arg=42</TT> - this policy specifies
 * that character.
...
public:
        ...
        [[maybe_unused]] const Parents&... parents) const
    {
        ...
        static_assert((owner_type::minimum_count() == 1) &&
                          (owner_type::maximum_count() == 1),
                      "Value separator support requires an owning node to have "
                      "a fixed count of 1");
        ...
        const auto separator_index =
            first_token.name.find_first_of(value_separator());
